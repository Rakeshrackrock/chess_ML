...
export const api = {
  ...
  joinGame: (joinCode: string) =>
    http<JoinResponse>("/join", {
      method: "POST",
      body: JSON.stringify({ joinCode }),
    }),

  getGame: (gameId: string) =>
    http<GetGameResponse>(`/games/${gameId}`),
  ...
  aiMove: (gameId: string, difficulty: string, playerToken?: string) =>
    http<MoveResponse>(`/games/${gameId}/ai-move`, {
      method: "POST",
      body: JSON.stringify({
        difficulty,
